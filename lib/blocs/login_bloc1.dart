...
class LoginBloc1 implements BaseBloc {
  final FakeRepository _repository;
  String _currentUser;
  CredentialModel _credentials = CredentialModel();

  LoginBloc1.withRepository(this._repository) {
...
//    });
  ...
  /* *******************************************************
    STREAMS
   ********************************************************/
  var _currentUserController = BehaviorSubject<ApiResponse1<UserModel>>();
  ...
  /* *******************************************************
    OUTPUT
   ********************************************************/
  Stream<ApiResponse1<UserModel>> get currentUserStream =>
  ...
  Stream<bool> get errorOccur => _currentUserController.stream.transform(
          StreamTransformer<ApiResponse1<UserModel>, bool>.fromHandlers(
...
//_currentUserController.sink.add(await _repository.getLocalUser());
  ...
  // loginPressed() async {
  //   _currentUserController.sink.add(ApiResponse1.loading('loading'));
  //   var response = await _repository.checkUser();
  ...
  //           ApiResponse1.error('Acest user nu are asociat nici o gestiune.'));
  //       return;
  //     }
